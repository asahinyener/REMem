...
and using paralel approaches of information fetching keep increasing the depth of search without significant cost to attention based context[RTQA] and yield higher accuracy at lower cost. Tool calling with python functions is a strong
fit for modern day LLMs due to abundance and variance of such data in the pretraining corpus[Tremu], #further integration of auto regressive generation and programming enviroments to produce harnesses for code execution with tool calling LLMs#,
modern day harnesses enable REPL like stateful interaction for code execution environments[dspy,RLM]. Work of [RLM] treats each turn to be composed of emitted code to be executed against the REPL, and supplies the REPL state and the produced output back to the LLM. Function that is of interest is calling sub-llms with their own scoped REPL environment. This enables the model to write the execution and retrieval stages as executed python code and have stateful returns of context from the sub-llm calls.
